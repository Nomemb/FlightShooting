...
public class BossWeapon : MonoBehaviour
{
    ...
    {
        // attackType 열거형의 이름과같은 코루틴을 실행
        StartCoroutine(attackType.ToString());
    }

    public void StopFiring(AttackType attackType)
    {
        // attackType 열거형의 이름과같은 코루틴을 중지
    ...
    private IEnumerator CircleFire()
    {
        ...
        while (true)
        {
            for( int i = 0; i< count; ++i)
            {
                ...
                // 발사체 이동 방향( 벡터 )
                float x = Mathf.Cos(angle * Mathf.PI / 180.0f); // Cos각도, 라디안 각도 표현을 위해 PI/180 곱함.
                float y = Mathf.Sin(angle * Mathf.PI / 180.0f); // Sin각도, 라디안 각도 표현을 위해 PI/180 곱함.
                // 발사체 이동 방향 설정
                clone.GetComponent<Movement2D>().MoveTo(new Vector2(x, y));
            }

            // 발사체가 시작되는 시작 각도 설정을 위한 변수
            weightAngle += 1;
